...
import EmployerPanelActions from '../EmployerPanel/actions';
//import { capitalize, getImageSrcByIndex, renderData } from '../../utils/utils';

const osname = platform();

class VacancyPanel extends React.Component {
            ...
            <Panel id={id}>
                ...
                <Group>
                    ...
                    <List>
                        ...
                        <Cell>
                            <InfoRow title="Требуемый опыт">
                                {data.experience && data.experience.name}
                            </InfoRow>
                        </Cell>
                        <Cell>
                            <InfoRow title="Занятость">
                                {this.renderEmploymentAndSchedule()}
                            </InfoRow>
                        </Cell>
                    </List>
                </Group>
                <Group>
                    <List>
                        <Cell
                            ...
                            expandable
                            onClick={(e) => {
                                this.props.actionsEmployerPanel.setPlaceId(data.employer.id);
                                this.props.go(e);
                            }}
                            data-to={'EmployerPanel'}
                        >
                            {data.employer && data.employer.name}
                        </Cell>
                        <Cell before={<Icon24Place/>} multiline>
                            {this.renderAddress()}
                        </Cell>
                    </List>
                    <Div>
                ...
                <Group>
                    <List>
                        ...
                        <Cell>
                            <InfoRow title="Занятость">
                                {data.employment && data.employment.name}
                            </InfoRow>
                        </Cell>
                    </List>
                </Group>
...
const mapStateToProps = store => {
    return {
        placeId: store.vacancyPanel.placeId,
        data: store.vacancyPanel.data,
...
export default connect(
    ...
    (dispatch) => {
        return {
            ...
            actionsEmployerPanel: bindActionCreators(EmployerPanelActions, dispatch),
        }
    }
)(VacancyPanel);
